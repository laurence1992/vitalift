...
export default function ClientProfile() {
  ...
  const load = async () => {
    ...
    if (assignmentRes.data?.program_id) {
      const { data: prog } = await supabase
        ...
        .eq("id", assignmentRes.data.program_id)
        .maybeSingle();
    ...
    // Load exercise names for workout detail
  ...
  const handleArchiveToggle = async () => {
    ...
    await supabase
      .from("profiles")
      .update({ status: newStatus, archived_at: newStatus === "archived" ? new Date().toISOString() : null } as any)
      .eq("id", client.id);
    ...
    return (
      <div className="min-h-screen pb-24">
        <div className="sticky top-0 z-40 flex items-center gap-3 border-b border-border bg-background/95 backdrop-blur-md px-4 py-3">
          ...
          <h1 className="text-base font-bold">{program ? "Edit Program" : "Create Program"}</h1>
        </div>
    ...
    return (
      <div className="min-h-screen pb-24">
        <div className="sticky top-0 z-40 flex items-center gap-3 border-b border-border bg-background/95 backdrop-blur-md px-4 py-3">
          ...
          <div>
            <h1 className="text-base font-bold">Workout Detail</h1>
