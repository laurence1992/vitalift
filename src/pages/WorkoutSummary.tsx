...
export default function WorkoutSummary() {
  ...
  const loadWorkout = async () => {
    ...
    const { data: day } = await supabase.from("program_days").select("label, day_note").eq("id", w.day_id).maybeSingle();
    if (day) { setDayLabel(day.day_note ? `${day.label} – ${day.day_note}` : day.label); } else { setDayLabel(w.day_id); }
    const { data: setsData } = await supabase.from("workout_sets").select("*").eq("workout_id", workoutId).order("set_number");
    setSets((setsData as WorkoutSetRow[]) || []);
    const exerciseIds = [...new Set((setsData || []).map((s: any) => s.exercise_id))];
    if (exerciseIds.length > 0) {
      const { data: exes } = await supabase.from("coach_exercises").select("id, name").in("id", exerciseIds);
        ...
        {Object.entries(byExercise).map(([exId, exSets]) => {
          ...
          const bestSet = exSets.reduce<{ weight: number; reps: number } | null>(
            (best, s) => {
              ...
              if (!best || s.weight > best.weight || (s.weight === best.weight && s.reps > best.reps)) return { weight: s.weight, reps: s.reps };
              return best;
            }, null
          );
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    ...
                    {bestSet && <span className="flex items-center gap-1"><Trophy className="h-3 w-3 text-primary" />{bestSet.weight}kg × {bestSet.reps}</span>}
                  </div>
                </div>
              </div>
